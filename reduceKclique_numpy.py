...
def filter_vertices_by_degree(G, k):
    ...
    return np.argwhere(np.sum(G.adjmat, 1) >= k - 1)
...
def filter_edges_by_degree(G, k):
    ...
    """
    combinations(range(G.adjmat.shape[0]), 2)

    return  zip(*[(u, v) for u, v in zip(G.adjmat.nonzero()) if np.logical_and(G.adjmat[u], G.adjmat[v]).count_nonzero() < k - 2])
...
def simplify(G, k, P):
    ...
    while True:
        ...
        E = tuple((u, v) for u, v in G.edges if len(G.neighbors[u] & G.neighbors[v]) < k - 2)
        if len(E):
